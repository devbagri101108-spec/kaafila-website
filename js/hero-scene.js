...
(function () {
  ...
  var ROPE = [
    [0, 640], [82, 670], [170, 701], [250, 725], [320, 745], [400, 752],
    [470, 733], [523, 703], [580, 674], [650, 660], [712, 673], [765, 700],
    [815, 728], [872, 738], [930, 723], [985, 701], [1045, 693], [1105, 707],
    [1165, 736], [1225, 769], [1288, 792], [1352, 790], [1420, 761],
    [1470, 723], [1498, 748], [1472, 802], [1402, 839], [1312, 856],
    [1218, 858], [1152, 848]
  ];
  ...
  function build(name) {
    ...
    /* Register the traced rope against this layout's bird and nest: the
       similarity that carries the banner's two landmarks onto the stage's.
       Because it carries the rotation too, a portrait phone — where the nest
       sits below the bird rather than beside it — gets the same run of rope,
       turned to follow the same line. */
    var rf = ROPE_FRAME;
    var bdx = rf.ball[0] - rf.bird[0], bdy = rf.ball[1] - rf.bird[1];
    var sdx = nest.ballX - L.bird.cx, sdy = nest.ballY - L.bird.cy;
    var ropeK = Math.hypot(sdx, sdy) / Math.hypot(bdx, bdy);
    var ropeA = Math.atan2(sdy, sdx) - Math.atan2(bdy, bdx);
    var rope = {
      k: ropeK, cos: Math.cos(ropeA) * ropeK, sin: Math.sin(ropeA) * ropeK,
      ox: L.bird.cx, oy: L.bird.cy, bx: rf.bird[0], by: rf.bird[1]
    };
  ...
  function towShape(s, t, p) {
    ...
    /* banner space -> stage space */
    var px = bx - R.bx, py = by - R.by;
    return [R.ox + px * R.cos - py * R.sin,
            R.oy + px * R.sin + py * R.cos];
  }
